...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (selectedEmployee && myEmployeeId) {
      const token = localStorage.getItem('token');
      fetch(`http://localhost:5000/api/chat/${selectedEmployee.id}?currentEmployeeId=${myEmployeeId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
        .then(res => res.json())
        .then(data => {
      ...
      <div className="flex-1 flex flex-col">
        {selectedEmployee ? (
              ...
              {filteredMessages.map(msg => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs md:max-w-md p-3 rounded-lg shadow-sm ${isMe ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none border border-gray-200'}`}>
                      {msg.content}
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50 text-gray-400 flex-col">
            ...
            <span>Select a colleague to chat.</span>
          </div>
        )}
      </div>
    </div>
